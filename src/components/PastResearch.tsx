...
const researchData: Research[] = [
  {
    ...
    items: [
      ...
      {
        name: "山本将叶",
        title: "大規模言語モデルによるSQLインジェクション攻撃の動的検知・防御に関する実証的検討",
        content: "SQLインジェクション（SQLi）攻撃に対し、先行研究では大規模言語モデル（LLM）を用いてWAFを突破する攻撃を生成し、それに基づき防御ルールを自動生成する手法が示された。本研究はこのアプローチを拡張し、防御ルールの生成サイクルを反復的に実行する動的な防御手法を提案する。この反復プロセスが、未知の亜種攻撃に対する検知精度や網羅性をいかに向上させるか、またその過程における過学習の可能性を実験的に検証する。これにより、継続的に変化する脅威へ自動追従する次世代のWebセキュリティ技術の確立を目指す。",
      },
    ],
  },
  {
